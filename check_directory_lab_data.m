% Modified to look for lab data format. - 04/10/19
% This small script looks in the directory and checks if the images are there.
%
...
if Nl > 0 % If the directory has calib_name* files...
    
    for pp = 1:Nl % 1:# of 'calib_name*' files in directory
        
        filenamepp =  l(pp).name; % Get a filename.
        if isempty(calib_name) % If no basename specified...
            iii = 1;
        else
            iii = findstr(filenamepp,calib_name); % Index to calib_name within filename.  Always 1?
        end

        loc_ext = findstr(filenamepp,format_image); % Index to file extension within filename.
        
        
        string_num = filenamepp(length_name+1:loc_ext - 2); % Leaves 'nnnn' of calib_name[nnnn].bmp.

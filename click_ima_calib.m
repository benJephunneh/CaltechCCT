...
% Modified to improve code: 11 Aug 19 (CPM)
fprintf(1,'\nProcessing image %d...\n',kk);
...
notGood = true; % Added by Caleb so he could re-do point selection on an image when he messes up:
while notGood
    figure(2);
    image(I);
    colormap(map);
...
% If you want to set the square dimensions for each image, change this to
% 'if true'.
if (~exist('dX', 'var') || (~exist('dY', 'var')) % This question is now asked only once

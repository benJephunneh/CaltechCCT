%Modified to open lab data format. - 04/01/19 (DY)
...
% addpath (genpath('J:\Tethers Unlimited\FabLab\MATLAB\'));
...
cell_list{1,1} = {'Standard (all the images are stored in memory)','calib_gui_normal_lab_data;'};
cell_list{2,1} = {'Memory efficient (the images are loaded one by one)','calib_gui_no_read;'};
cell_list{3,1} = {'Exit',['disp(''Bye. To run again, type calib_gui_lab_data.''); close(' num2str(fig_number) ');']};

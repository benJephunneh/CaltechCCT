...
while strcmpi(format_image2, '0')
   
    format_image2 =  input('Image format: ([]=''r''=''ras'', ''b''=''bmp'', ''t''=''tif'', ''p''=''pgm'', ''j''=''jpg'', ''m''=''ppm'', ''d''=''dat'') ','s');
	
% 	if isempty(format_image2),
%    	format_image2 = 'ras';
% 	end;

    switch lower(format_image2)
        case 'm'
            format_image2 = 'ppm';
        case 'b'
            format_image2 = 'bmp';
...
%%% READ IN IMAGE: % Note: This if-else-end block may have been created
%%% when MATLAB's 'imread' function couldn't handle all of these image
%%% formats.  It seems unnecessary, where one line can handle all of these
%%% formats, save our lab_data format. (13 Aug 19, CPM)

% if strcmpi(format_image2, 'ppm') % if format_image2(2) == 'p',
if strcmpi(format_image2, 'dat')
    I = read_lab_data(ima_name);
else
    I = double(imread(ima_name)); % I = double(loadppm(ima_name));
end
% elseif strcmpi(format_image2, 'pgm')
...
if size(I,3) > 1 % If the image is RGB?
   I = I(:,:,2); % Make the image only G?
end

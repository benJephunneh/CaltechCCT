...
calib_file_name_left = input('Name of the left camera calibration file ([]=camera_results_v1.mat): ','s');

if isempty(calib_file_name_left)
    calib_file_name_left = 'camera_results_v1.mat';
end

calib_file_name_right = input('Name of the right camera calibration file ([]=camera_results_v2.mat): ','s');
if isempty(calib_file_name_right)
    calib_file_name_right = 'camera_results_v2.mat';
end

% CPM: Removing in favor of try-catch.
% if ~exist(calib_file_name_left, 'file') || ~exist(calib_file_name_right, 'file')
%     fprintf(1,'Error: left and right calibration files do not exist.\n');
%     return;
% end

%----------Load right camera's results----------%
fprintf(1,'Loading the left camera''s calibration result file %s...\n', calib_file_name_left);
clear calib_name
...
%----------Load right camera's results----------%
fprintf(1,'Loading the right camera''s calibration result file %s...\n', calib_file_name_right);
clear calib_name
